...
def back_to_modeFrame():
    ...
    create_modeFrame_easy_hard_back()
...
def create_menuFrame_start_exit():
    ...
    startGame_btn = Button(menuFrame,
                        ...
                        command = create_modeFrame_easy_hard_back) # เมื่อกดปุ่ม Start จะไปทำงานที่ create_modes_btn()
...
################################################################## Modes Button
# function สำหรับสร้างปุ่มเลือก Mode การเล่น Hard, Easy เพื่อแสดงบน menuFrame
def create_modeFrame_easy_hard_back():
    ...
    text_show = Label(modeFrame,
                        ...
                        fg = _random_rgb(), # function ไหนที่มีการ return... ไม่ว่า function นั้นจะไม่มี paramiter ก็ตาม ถ้าเรียกใช้ function นั้นต้องใส่ ()
                        font=50)
    text_show.pack(pady=30) # แสดงบนหน้าจอโดยเว้นระยะห่าง บน*ล่าง 30*30
    ...
    easyMode_btn = Button(modeFrame,
                        ...
                        command = lambda: create_gameFrame(1)) # เมื่อกดปุ่ม EASY จะไปทำงานที่ createGame_frame() โดยส่ง
    easyMode_btn.pack(pady=30) # แสดงบนหน้าจอโดยเว้นระยะห่าง บน*ล่าง 30*30
    ...
    hardMode_btn = Button(modeFrame,
                    ...
                    bg="coral4",
                    #     เมื่อกดปุ่มแล้วต้องการให้ไปทำงาน function ที่ต้องส่ง paramiter ไปด้วน --> จะต้องใช้ lambda เพื่อกำหนดว่า function นี้สามารถส่ง paramiter ไปได้
                    command = lambda: create_gameFrame(2)) # เมื่อกดปุ่ม HART จะไปทำงานที่ createGame_frame() 
    hardMode_btn.pack(pady=30) # แสดงบนหน้าจอโดยเว้นระยะห่าง บน*ล่าง 30*30
    ...
    back_btn.pack(pady=30) # แสดงบนหน้าจอโดยเว้นระยะห่าง บน*ล่าง 30*30



################################################################## Game Frame
# function สำหรับสร้างหน้าเล่นเกมสุ่มเลข โดยรับ mode เข้ามาว่า ง่าย(1) หรือ ยาก(2)
def create_gameFrame(mode):
    # กำหนดให้ตัวแปลต่างๆนี้ สามารถเรียกใช้งานได้จาก function อื่นๆ
    global number_of_times_to_play, traget
    ...
    if mode == 1:
        # สุ่มตัวเลขจำนวนเต็ม 1-10
        traget = random.randint(1,10)
    ...
    # ถ้าเลือก gmae mode ยาก
        ...
        # สุ่มตัวเลขจำนวนเต็ม 1-30
        traget = random.randint(1,30)
        # กำหนดให้จำนวนครั้งที่สามารถเล่นอกมได้
...
def game(txt, number_of_rows_of_button):
    ...
    text_show_1 = Label(gameFrame,
                      text="เลือกตัวเลขที่คุณคิดว่าถูดต้องในระหว่าง " + txt,
                      fg = _random_rgb(), # function ไหนที่มีการ return... ไม่ว่า function นั้นจะไม่มี paramiter ก็ตาม ถ้าเรียกใช้ function นั้นต้องใส่ ()
                      font=50)
    ...
    # ตัวเลขสำหรับแสดงบนปุ่มคำตอบ
    answer_number = 1
    # for สำหรับสร้างปุ่มในการกดตัวเลขตั้งแต่ 1-10 และ 1-30 เพื่อแสดงบน gameFrame
    # จำนวน row ที่ต้องสร้าง โดยเริ่มต้นตั้งแต่แถวที่ 2 ไปจนถึงแถวที่ ... + 2 (ถ้าเป็นเลข 1-10 จะมี 2 แถว แต่ถ้าเป็นเลข 1-30 จะมี 6 แถว และทุกแถวจะมี 5 column)
    for r in range(2, number_of_rows_of_button+2, 1):
        # for สำหรับสร้างปุ่มตามจำนวน column 0-4
        for c in range(0, 5, 1):
            # สร้างปุ่ม และแสดงบน gameFrame
            Button(gameFrame,
                   text=str(answer_number),
                   font=10,
                   width=5,
                   bg = _random_rgb(),
                   #     เมื่อกดปุ่มแล้วต้องการให้ไปทำงาน function ที่ต้องส่ง paramiter ไปด้วน --> จะต้องใช้ lambda เพื่อกำหนดว่า function นี้สามารถส่ง paramiter ไปได้
...
def answer_processing(answer_number):
    ...
    if number_of_times_to_play > 0:
        ...
        # ถ้าทายตัวเลขที่ มากเกินไป
            ...
            # เปลี่ยนข้อความที่แสดงบนหน้าจอ
            text_show_3.config(text="หมายเลย " + str(answer_number) + " มาก เกินไป")
        # ถ้าทายตัวเลขที่ น้ายเกินไป
        elif answer_number < traget:
            # เปลี่ยนข้อความที่แสดงบนหน้าจอ
            text_show_3.config(text="หมายเลย " + str(answer_number) + " น้อย เกินไป")
...
def create_endFrame(isWin, answer_number):
    ...
    if isWin:
        end_text_show_1.config(text="YOU WIN!!",
                               bg="chartreuse3")
        end_text_show_2.config(text="หมายเลข " + str(answer_number) + " เป็นคำตอบที่ถูกต้อง")
    # ถ้าผู้เล่นทายเลขไม่ถูกเลย จะแสดงข้อความ
    else:
        end_text_show_1.config(text="YOU LOSE!!",
                               fg = "gainsboro",
                               bg = "firebrick1")
        end_text_show_2.config(text="หมายเลขที่ถูกต้องคือ " + str(traget))
